...
typedef std::pair<time_t, unsigned long int>
    time_pair; /* (seconds, nanoseconds) */
...
/*
  ...
  This could surely be done more cleverly.  Or already has been.  (Cf. boost
  filesystem)
...
public:
  ...
  Leaf(const std::string &password, const std::string base_name = std::string(),
       const std::string dir_name = std::string(), const bool do_load = true);
  virtual ~Leaf();
...
public:
  ...
  */
  ...
  /*
    ...
    bug some day.
  */
  bool operator<=(const LeafProxy &rhs) const { return key() <= rhs.key(); }
  ...
  void key_cache(const std::string &in) {
    cached_key = in;
    validate();
  }
...
public:
  LeafMatcher(const srd::vector_string &, const srd::vector_string &,
              bool conj);
...
struct StringMatcher {
  // virtual const bool operator()(const std::string &in_a, const std::string
  // &in_b) const = 0;
  virtual const bool operator()(const std::string &in_a,
                                const std::string &in_b) const {
    std::cerr << "():  Fail base" << std::endl;
    return false;
  };
  // virtual const bool contains(const std::string &in_a, const std::string
  // &in_b) const = 0;
  virtual const bool contains(const std::string &in_a,
                              const std::string &in_b) const {
    std::cerr << "contains:  Fail base" << std::endl;
    return false;
  };
...
struct UpperStringMatcher : public StringMatcher {
  virtual const bool operator()(const std::string &in_a,
                                const std::string &in_b) const {
    return boost::algorithm::to_upper_copy(in_a) ==
           boost::algorithm::to_upper_copy(in_b);
  }
  virtual const bool contains(const std::string &in_a,
                              const std::string &in_b) const {
    return boost::algorithm::to_upper_copy(in_a).find(
               boost::algorithm::to_upper_copy(in_b)) != std::string::npos;
  }
...
class LeafProxyMap {
  ...
  // LeafProxyMapInternalType::value_type>
...
public:
  ...
  LeafProxyMap(const LeafProxyMap &lpm) { the_map = lpm.the_map; }
  ...
  LeafProxyMap filter_keys(const srd::vector_string &, const bool exact,
...
public:
  Root(const std::string &password, const std::string path = std::string(),
  ...
  void set_leaf(const std::string &proxy_key, const std::string &key,
