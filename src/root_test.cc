...
int test_root_basic(string password) {
...

  {
    ...
    for (vector_string::iterator it = messages.begin(); it != messages.end();
         it++) {
      ostringstream ss;
      ss << it->size();
      string key(ss.str());
  ...
  cout << "Re-instantiating root." << endl;
  {
    ...
    for (vector_string::iterator it = messages.begin(); it != messages.end();
      ...
      LeafProxyMap results =
          root.filter_payloads(payloads_to_find, true, IdentStringMatcher());
      if (0 == results.size())
        error_count++;
    }
  }
...
int confirm_once(Root &root, pair<string, string> text) {
  ...
  LeafProxyMap key_results =
      root.filter_keys(key_pattern, false, IdentStringMatcher());
  if (key_results.size() != 1) {
    cout << "Key \"" << text.first << ":  "
         << "Found " << key_results.size() << " keys expected 1." << endl;
    ret++;
  }

  vector_string payload_pattern;
  payload_pattern.push_back(text.second);
  LeafProxyMap payload_results =
      root.filter_payloads(payload_pattern, true, IdentStringMatcher());
  ...
  payload_results =
      root.filter_keys(key_pattern, false, IdentStringMatcher())
          .filter_payloads(payload_pattern, true, IdentStringMatcher());
  if (payload_results.size() != 1) {
...
int test_root_singles(const map<string, string> &text) {
  ...
  string password = pseudo_random_string(15);
  {
    ...
    int errors = count_if(text.begin(), text.end(),
...
int confirm_twice(Root &root, const pair<string, string> &text) {
  ...
  key_pattern.push_back(text.first);
  LeafProxyMap key_results =
      root.filter_keys(key_pattern, false, UpperStringMatcher());
  if (key_results.size() != 2) {
    cout << "Key \"" << text.first << ":  "
         << "Found " << key_results.size() << " keys expected 2." << endl;
    ret++;
  }
  ...
  payload_pattern.push_back(text.second);
  LeafProxyMap payload_results =
      root.filter_payloads(payload_pattern, true, UpperStringMatcher());
...
bool confirm_ordering(Root &root) {
  ...
  for (LeafProxyMap::LPM_Set::iterator it = leaves.begin(); it != leaves.end();
       it++) {
...
int test_root_change_password() {
  ...
  cout << "Re-instantiating root and changing password." << endl;
  {
    ...
    for (vector_string::iterator it = messages.begin(); it != messages.end();
      ...
      vector_string payloads_to_find;
      payloads_to_find.push_back(*it);
      LeafProxyMap results = new_root.filter_payloads(payloads_to_find, true,
                                                      IdentStringMatcher());
      if (0 == results.size())
        error_count++;
    }
